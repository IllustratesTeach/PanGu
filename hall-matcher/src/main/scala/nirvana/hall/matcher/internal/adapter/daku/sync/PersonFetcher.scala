...
class PersonFetcher(implicit dataSource: DataSource) extends SyncDataFetcher{
   override val MAX_SEQ_SQL: String = "select max(t.seq) as seq from gafis_person t"
   override val MIN_SEQ_SQL: String = "select min(t.seq) as seq from gafis_person t where t.seq > "
   /** 同步人员基本信息 */
