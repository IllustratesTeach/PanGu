...
//  s"where ${wrapUpdateTimeAsLong()}  >"
  ...
  override val SYNC_SQL = s"select t.ora_sid as sid, ${wrapModTimeAsLong()} as seq from normallp_latfinger_mod t where ${wrapModTimeAsLong()} >=? and ${wrapModTimeAsLong()} <=? order by t.modtime"
  /** 同步现场指纹 */
