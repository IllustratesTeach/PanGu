...
class TemplatePalmFetcher(hallMatcherConfig: HallMatcherConfig, dataSource: DataSource) extends SyncDataFetcher(hallMatcherConfig, dataSource){
  ...
  override val MIN_SEQ_SQL: String = "select min(t.updatetime) from normaltp_tpcardinfo t where t.seq >"
  override val SYNC_SQL: String = "select * from (select t.ora_sid as sid, t.palmlmnt, t.palmrmnt, t.updatetime as seq from normaltp_tpcardinfo t  where t.updatetime >=? order by t.updatetime) tt where rownum <=? "
