...
class AddPersonInfoRequestFilterTest extends BaseServiceTestSupport {
  ...
  def test_addPersonInfo: Unit ={
    ...
    val protobufRequest1 = BaseCommand.newBuilder().setToken("asdf").setVersion(103)
    ...
    val protobufRequest2 = BaseCommand.newBuilder().setTaskId(1)
    protobufRequest2.setExtension(UpdatePersonRequest.cmd ,updateRequest.build())
    val protobufResponse2 = BaseCommand.newBuilder()
    protobufResponse2.setStatus(CommandStatus.OK)
