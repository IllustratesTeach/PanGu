...
class Fpt5Test extends BaseV70TestCase{
  ...
  def test_fpt5_tpget(): Unit = {
    ...
    fPT5File.fingerprintPackage.add(service.getFingerprintPackage("R5224010109992013070451"))
  ...
  def test_fpt5_tpadd(): Unit = {
    ...
    val originalList = XmlLoader.parseXML[FPT5File](new String(IOUtils.toByteArray(getClass.getResourceAsStream("/R5224010109992013070451.xml"))))

    for(i <- 0 until originalList.fingerprintPackage.size()){
      service.addFingerprintPackage(originalList.fingerprintPackage.get(i))
    }
