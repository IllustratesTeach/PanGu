 ...
 */
class JdbcTemplate(dataSource: DataSource, transactionManager: PlatformTransactionManager) {
  private val javaTemplate = new org.springframework.jdbc.core.JdbcTemplate(dataSource)

  def withTransaction[T](action: JdbcTemplate => T, transactionDefinition: TransactionDefinition = new DefaultTransactionDefinition()): T = {
    new TransactionTemplate(transactionManager, transactionDefinition).execute { transaction =>
      action(this)
    }
  }
