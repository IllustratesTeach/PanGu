...
class SyncDataFilter(httpServletRequest: HttpServletRequest,
                     syncTPCardService: SyncTPCardService,
                     syncLPCardService: SyncLPCardService,
                     syncLPPalmService: SyncLPPalmService,
                     syncCaseInfoService: SyncCaseInfoService) extends RpcServerMessageFilter{
  override def handle(commandRequest: BaseCommand, commandResponse: CommandResponse, handler: RpcServerMessageHandler): Boolean ={
    if(commandRequest.hasExtension(SyncTPCardRequest.cmd)) {
      ...
      val dbId = getDBID(httpServletRequest)
      syncLPPalmService.syncLPCard(responseBuilder, request.getTimestamp, request.getSize, dbId)
