  ...
  override def doWork(): Unit = {
    HallFetchConfig.find_by_deletag("1").toSeq.foreach{ fetchConfig=>
      ...
      fetchConfig.typ match {
        ...
        case x if(x.equals(HallApiConstants.SYNC_TYPE_TPCARD) || x.equals(HallApiConstants.SYNC_TYPE_LPCARD)
          || x.equals(HallApiConstants.SYNC_TYPE_CASEINFO) || x.equals(HallApiConstants.SYNC_TYPE_LPPALM)) =>
        case other =>
      ...
      try{
        while(sidIter.hasNext){
          ...
          if(baseResponse.getStatus == CommandStatus.OK) {
            ...
            if (response.getMatchStatus.equals(MatchStatus.UN_KNOWN) && response.getMatchResult.getCandidateNum==0) {
              //该条任务7.0中创建失败
              ...
              syncInfoLogManageService.recordSyncDataIdentifyLog(uuid, String.valueOf(tmp), fetchConfig.typ, fetchConfig.url.substring(7,fetchConfig.url.length-5) ,"0","1")
            } else {
