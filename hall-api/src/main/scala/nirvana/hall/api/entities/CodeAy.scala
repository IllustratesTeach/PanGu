...
object CodeAy extends SQLSyntaxSupport[CodeAy] {
 ...
 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()
  ...
  def find(code: String, name: Option[String], deleteFlag: Option[String], ord: Option[Long], remark: Option[String], firstLetter: Option[String])(implicit session: DBSession = autoSession): Option[CodeAy] = {
    withSQL {
      select.from(CodeAy as ca).where.eq(ca.code, code).and.eq(ca.name, name).and.eq(ca.deleteFlag, deleteFlag).and.eq(ca.ord, ord).and.eq(ca.remark, remark).and.eq(ca.firstLetter, firstLetter)
  ...
  def destroy(entity: CodeAy)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(CodeAy).where.eq(column.code, entity.code).and.eq(column.name, entity.name).and.eq(column.deleteFlag, entity.deleteFlag).and.eq(column.ord, entity.ord).and.eq(column.remark, entity.remark).and.eq(column.firstLetter, entity.firstLetter) }.update.apply()
  }
