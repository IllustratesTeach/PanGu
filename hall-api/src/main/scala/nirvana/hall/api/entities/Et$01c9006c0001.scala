...
case class Et$01c9006c0001(
  ...
  def save()(implicit session: DBSession = Et$01c9006c0001.autoSession): Et$01c9006c0001 = Et$01c9006c0001.save(this)(session)

  def destroy()(implicit session: DBSession = Et$01c9006c0001.autoSession): Unit = Et$01c9006c0001.destroy(this)(session)

}


object Et$01c9006c0001 extends SQLSyntaxSupport[Et$01c9006c0001] {

  override val tableName = "ET$01C9006C0001"
  ...
  def apply(e: SyntaxProvider[Et$01c9006c0001])(rs: WrappedResultSet): Et$01c9006c0001 = apply(e.resultName)(rs)
  def apply(e: ResultName[Et$01c9006c0001])(rs: WrappedResultSet): Et$01c9006c0001 = new Et$01c9006c0001(
    pkId = rs.get(e.pkId),
  ...
  val e = Et$01c9006c0001.syntax("e")

 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()

  def find(pkId: Option[String], personId: Option[String], fgp: Option[Short], groupId: Option[Short], lobtype: Option[Short], inputpsn: Option[String], inputtime: Option[DateTime], modifiedpsn: Option[String], modifiedtime: Option[DateTime], fgpCase: Option[String], auditStatus: Option[Short], description: Option[String], mainPattern: Option[Short], vicePattern: Option[Short], seq: Option[Long], fingerDataNosqlId: Option[String], gatherData: Option[Blob])(implicit session: DBSession = autoSession): Option[Et$01c9006c0001] = {
    withSQL {
      select.from(Et$01c9006c0001 as e).where.eq(e.pkId, pkId).and.eq(e.personId, personId).and.eq(e.fgp, fgp).and.eq(e.groupId, groupId).and.eq(e.lobtype, lobtype).and.eq(e.inputpsn, inputpsn).and.eq(e.inputtime, inputtime).and.eq(e.modifiedpsn, modifiedpsn).and.eq(e.modifiedtime, modifiedtime).and.eq(e.fgpCase, fgpCase).and.eq(e.auditStatus, auditStatus).and.eq(e.description, description).and.eq(e.mainPattern, mainPattern).and.eq(e.vicePattern, vicePattern).and.eq(e.seq, seq).and.eq(e.fingerDataNosqlId, fingerDataNosqlId).and.eq(e.gatherData, gatherData)
    }.map(Et$01c9006c0001(e.resultName)).single.apply()
  }

  def findAll()(implicit session: DBSession = autoSession): List[Et$01c9006c0001] = {
    withSQL(select.from(Et$01c9006c0001 as e)).map(Et$01c9006c0001(e.resultName)).list.apply()
  }

  def countAll()(implicit session: DBSession = autoSession): Long = {
    withSQL(select(sqls.count).from(Et$01c9006c0001 as e)).map(rs => rs.long(1)).single.apply().get
  }

  def findBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Option[Et$01c9006c0001] = {
    withSQL {
      select.from(Et$01c9006c0001 as e).where.append(where)
    }.map(Et$01c9006c0001(e.resultName)).single.apply()
  }

  def findAllBy(where: SQLSyntax)(implicit session: DBSession = autoSession): List[Et$01c9006c0001] = {
    withSQL {
      select.from(Et$01c9006c0001 as e).where.append(where)
    }.map(Et$01c9006c0001(e.resultName)).list.apply()
  }

  def countBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Long = {
    withSQL {
      select(sqls.count).from(Et$01c9006c0001 as e).where.append(where)
  ...
  def create(
    ...
    seq: Option[Long] = None,
    fingerDataNosqlId: Option[String] = None,
    gatherData: Option[Blob] = None)(implicit session: DBSession = autoSession): Et$01c9006c0001 = {
    withSQL {
      insert.into(Et$01c9006c0001).columns(
        column.pkId,
        column.personId,
        column.fgp,
        column.groupId,
  ...
  def save(entity: Et$01c9006c0001)(implicit session: DBSession = autoSession): Et$01c9006c0001 = {
    withSQL {
      update(Et$01c9006c0001).set(
