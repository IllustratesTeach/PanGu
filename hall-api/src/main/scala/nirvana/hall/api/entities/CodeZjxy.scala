...
object CodeZjxy extends SQLSyntaxSupport[CodeZjxy] {
 ...
 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()
  ...
  def find(code: String)(implicit session: DBSession = autoSession): Option[CodeZjxy] = {
    withSQL {
      select.from(CodeZjxy as cz).where.eq(cz.code, code)
  ...
  def destroy(entity: CodeZjxy)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(CodeZjxy).where.eq(column.code, entity.code) }.update.apply()
  }
