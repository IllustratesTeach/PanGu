...
case class GafisGatherCityCode5204(
  pkId: String,
  ...
  def save()(implicit session: DBSession = GafisGatherCityCode5204.autoSession): GafisGatherCityCode5204 = GafisGatherCityCode5204.save(this)(session)

  def destroy()(implicit session: DBSession = GafisGatherCityCode5204.autoSession): Unit = GafisGatherCityCode5204.destroy(this)(session)

}


object GafisGatherCityCode5204 extends SQLSyntaxSupport[GafisGatherCityCode5204] {

  override val tableName = "GAFIS_GATHER__CITY_CODE_520_4"
  ...
  def apply(ggcc: SyntaxProvider[GafisGatherCityCode5204])(rs: WrappedResultSet): GafisGatherCityCode5204 = apply(ggcc.resultName)(rs)
  def apply(ggcc: ResultName[GafisGatherCityCode5204])(rs: WrappedResultSet): GafisGatherCityCode5204 = new GafisGatherCityCode5204(
  ...
  val ggcc = GafisGatherCityCode5204.syntax("ggcc")

 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()

  def find(pkId: String, personId: Option[String], fgp: Short, groupId: Option[Short], lobtype: Short, inputpsn: Option[String], inputtime: DateTime, modifiedpsn: Option[String], modifiedtime: Option[DateTime], fgpCase: Option[String], auditStatus: Option[Short], description: Option[String], mainPattern: Option[Short], vicePattern: Option[Short], gatherData: Blob, seq: Option[Long], fingerDataNosqlId: Option[String], partitionCitycode: Option[String])(implicit session: DBSession = autoSession): Option[GafisGatherCityCode5204] = {
    withSQL {
      select.from(GafisGatherCityCode5204 as ggcc).where.eq(ggcc.pkId, pkId).and.eq(ggcc.personId, personId).and.eq(ggcc.fgp, fgp).and.eq(ggcc.groupId, groupId).and.eq(ggcc.lobtype, lobtype).and.eq(ggcc.inputpsn, inputpsn).and.eq(ggcc.inputtime, inputtime).and.eq(ggcc.modifiedpsn, modifiedpsn).and.eq(ggcc.modifiedtime, modifiedtime).and.eq(ggcc.fgpCase, fgpCase).and.eq(ggcc.auditStatus, auditStatus).and.eq(ggcc.description, description).and.eq(ggcc.mainPattern, mainPattern).and.eq(ggcc.vicePattern, vicePattern).and.eq(ggcc.gatherData, gatherData).and.eq(ggcc.seq, seq).and.eq(ggcc.fingerDataNosqlId, fingerDataNosqlId).and.eq(ggcc.partitionCitycode, partitionCitycode)
    }.map(GafisGatherCityCode5204(ggcc.resultName)).single.apply()
  }

  def findAll()(implicit session: DBSession = autoSession): List[GafisGatherCityCode5204] = {
    withSQL(select.from(GafisGatherCityCode5204 as ggcc)).map(GafisGatherCityCode5204(ggcc.resultName)).list.apply()
  }

  def countAll()(implicit session: DBSession = autoSession): Long = {
    withSQL(select(sqls.count).from(GafisGatherCityCode5204 as ggcc)).map(rs => rs.long(1)).single.apply().get
  }

  def findBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Option[GafisGatherCityCode5204] = {
    withSQL {
      select.from(GafisGatherCityCode5204 as ggcc).where.append(where)
    }.map(GafisGatherCityCode5204(ggcc.resultName)).single.apply()
  }

  def findAllBy(where: SQLSyntax)(implicit session: DBSession = autoSession): List[GafisGatherCityCode5204] = {
    withSQL {
      select.from(GafisGatherCityCode5204 as ggcc).where.append(where)
    }.map(GafisGatherCityCode5204(ggcc.resultName)).list.apply()
  }

  def countBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Long = {
    withSQL {
      select(sqls.count).from(GafisGatherCityCode5204 as ggcc).where.append(where)
  ...
  def create(
    ...
    seq: Option[Long] = None,
    fingerDataNosqlId: Option[String] = None,
    partitionCitycode: Option[String] = None)(implicit session: DBSession = autoSession): GafisGatherCityCode5204 = {
    withSQL {
      insert.into(GafisGatherCityCode5204).columns(
        column.pkId,
        column.personId,
        column.fgp,
        column.groupId,
  ...
  def save(entity: GafisGatherCityCode5204)(implicit session: DBSession = autoSession): GafisGatherCityCode5204 = {
    withSQL {
      update(GafisGatherCityCode5204).set(
  ...
  def destroy(entity: GafisGatherCityCode5204)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(GafisGatherCityCode5204).where.eq(column.pkId, entity.pkId).and.eq(column.personId, entity.personId).and.eq(column.fgp, entity.fgp).and.eq(column.groupId, entity.groupId).and.eq(column.lobtype, entity.lobtype).and.eq(column.inputpsn, entity.inputpsn).and.eq(column.inputtime, entity.inputtime).and.eq(column.modifiedpsn, entity.modifiedpsn).and.eq(column.modifiedtime, entity.modifiedtime).and.eq(column.fgpCase, entity.fgpCase).and.eq(column.auditStatus, entity.auditStatus).and.eq(column.description, entity.description).and.eq(column.mainPattern, entity.mainPattern).and.eq(column.vicePattern, entity.vicePattern).and.eq(column.gatherData, entity.gatherData).and.eq(column.seq, entity.seq).and.eq(column.fingerDataNosqlId, entity.fingerDataNosqlId).and.eq(column.partitionCitycode, entity.partitionCitycode) }.update.apply()
  }
