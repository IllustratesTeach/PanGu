...
case class GafisGatherCityCode5223(
  pkId: String,
  ...
  def save()(implicit session: DBSession = GafisGatherCityCode5223.autoSession): GafisGatherCityCode5223 = GafisGatherCityCode5223.save(this)(session)

  def destroy()(implicit session: DBSession = GafisGatherCityCode5223.autoSession): Unit = GafisGatherCityCode5223.destroy(this)(session)

}


object GafisGatherCityCode5223 extends SQLSyntaxSupport[GafisGatherCityCode5223] {

  override val tableName = "GAFIS_GATHER__CITY_CODE_522_3"
  ...
  def apply(ggcc: SyntaxProvider[GafisGatherCityCode5223])(rs: WrappedResultSet): GafisGatherCityCode5223 = apply(ggcc.resultName)(rs)
  def apply(ggcc: ResultName[GafisGatherCityCode5223])(rs: WrappedResultSet): GafisGatherCityCode5223 = new GafisGatherCityCode5223(
  ...
  val ggcc = GafisGatherCityCode5223.syntax("ggcc")

 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()

  def find(pkId: String, personId: Option[String], fgp: Short, groupId: Option[Short], lobtype: Short, inputpsn: Option[String], inputtime: DateTime, modifiedpsn: Option[String], modifiedtime: Option[DateTime], fgpCase: Option[String], auditStatus: Option[Short], description: Option[String], mainPattern: Option[Short], vicePattern: Option[Short], gatherData: Blob, seq: Option[Long], fingerDataNosqlId: Option[String], partitionCitycode: Option[String])(implicit session: DBSession = autoSession): Option[GafisGatherCityCode5223] = {
    withSQL {
      select.from(GafisGatherCityCode5223 as ggcc).where.eq(ggcc.pkId, pkId).and.eq(ggcc.personId, personId).and.eq(ggcc.fgp, fgp).and.eq(ggcc.groupId, groupId).and.eq(ggcc.lobtype, lobtype).and.eq(ggcc.inputpsn, inputpsn).and.eq(ggcc.inputtime, inputtime).and.eq(ggcc.modifiedpsn, modifiedpsn).and.eq(ggcc.modifiedtime, modifiedtime).and.eq(ggcc.fgpCase, fgpCase).and.eq(ggcc.auditStatus, auditStatus).and.eq(ggcc.description, description).and.eq(ggcc.mainPattern, mainPattern).and.eq(ggcc.vicePattern, vicePattern).and.eq(ggcc.gatherData, gatherData).and.eq(ggcc.seq, seq).and.eq(ggcc.fingerDataNosqlId, fingerDataNosqlId).and.eq(ggcc.partitionCitycode, partitionCitycode)
    }.map(GafisGatherCityCode5223(ggcc.resultName)).single.apply()
  }

  def findAll()(implicit session: DBSession = autoSession): List[GafisGatherCityCode5223] = {
    withSQL(select.from(GafisGatherCityCode5223 as ggcc)).map(GafisGatherCityCode5223(ggcc.resultName)).list.apply()
  }

  def countAll()(implicit session: DBSession = autoSession): Long = {
    withSQL(select(sqls.count).from(GafisGatherCityCode5223 as ggcc)).map(rs => rs.long(1)).single.apply().get
  }

  def findBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Option[GafisGatherCityCode5223] = {
    withSQL {
      select.from(GafisGatherCityCode5223 as ggcc).where.append(where)
    }.map(GafisGatherCityCode5223(ggcc.resultName)).single.apply()
  }

  def findAllBy(where: SQLSyntax)(implicit session: DBSession = autoSession): List[GafisGatherCityCode5223] = {
    withSQL {
      select.from(GafisGatherCityCode5223 as ggcc).where.append(where)
    }.map(GafisGatherCityCode5223(ggcc.resultName)).list.apply()
  }

  def countBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Long = {
    withSQL {
      select(sqls.count).from(GafisGatherCityCode5223 as ggcc).where.append(where)
  ...
  def create(
    ...
    seq: Option[Long] = None,
    fingerDataNosqlId: Option[String] = None,
    partitionCitycode: Option[String] = None)(implicit session: DBSession = autoSession): GafisGatherCityCode5223 = {
    withSQL {
      insert.into(GafisGatherCityCode5223).columns(
        column.pkId,
        column.personId,
        column.fgp,
        column.groupId,
  ...
  def save(entity: GafisGatherCityCode5223)(implicit session: DBSession = autoSession): GafisGatherCityCode5223 = {
    withSQL {
      update(GafisGatherCityCode5223).set(
  ...
  def destroy(entity: GafisGatherCityCode5223)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(GafisGatherCityCode5223).where.eq(column.pkId, entity.pkId).and.eq(column.personId, entity.personId).and.eq(column.fgp, entity.fgp).and.eq(column.groupId, entity.groupId).and.eq(column.lobtype, entity.lobtype).and.eq(column.inputpsn, entity.inputpsn).and.eq(column.inputtime, entity.inputtime).and.eq(column.modifiedpsn, entity.modifiedpsn).and.eq(column.modifiedtime, entity.modifiedtime).and.eq(column.fgpCase, entity.fgpCase).and.eq(column.auditStatus, entity.auditStatus).and.eq(column.description, entity.description).and.eq(column.mainPattern, entity.mainPattern).and.eq(column.vicePattern, entity.vicePattern).and.eq(column.gatherData, entity.gatherData).and.eq(column.seq, entity.seq).and.eq(column.fingerDataNosqlId, entity.fingerDataNosqlId).and.eq(column.partitionCitycode, entity.partitionCitycode) }.update.apply()
  }
