...
object CodeAjzt extends SQLSyntaxSupport[CodeAjzt] {
 ...
 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()
  ...
  def find(code: String)(implicit session: DBSession = autoSession): Option[CodeAjzt] = {
    withSQL {
      select.from(CodeAjzt as ca).where.eq(ca.code, code)
  ...
  def destroy(entity: CodeAjzt)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(CodeAjzt).where.eq(column.code, entity.code) }.update.apply()
  }
