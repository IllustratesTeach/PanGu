...
object CodeXctqfs extends SQLSyntaxSupport[CodeXctqfs] {
 ...
 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()
  ...
  def find(code: String)(implicit session: DBSession = autoSession): Option[CodeXctqfs] = {
    withSQL {
      select.from(CodeXctqfs as cx).where.eq(cx.code, code)
  ...
  def destroy(entity: CodeXctqfs)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(CodeXctqfs).where.eq(column.code, entity.code) }.update.apply()
  }
