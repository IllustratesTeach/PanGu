...
case class GafisGatherCityCode5202(
  pkId: String,
  ...
  def save()(implicit session: DBSession = GafisGatherCityCode5202.autoSession): GafisGatherCityCode5202 = GafisGatherCityCode5202.save(this)(session)

  def destroy()(implicit session: DBSession = GafisGatherCityCode5202.autoSession): Unit = GafisGatherCityCode5202.destroy(this)(session)

}


object GafisGatherCityCode5202 extends SQLSyntaxSupport[GafisGatherCityCode5202] {

  override val tableName = "GAFIS_GATHER__CITY_CODE_520_2"
  ...
  def apply(ggcc: SyntaxProvider[GafisGatherCityCode5202])(rs: WrappedResultSet): GafisGatherCityCode5202 = apply(ggcc.resultName)(rs)
  def apply(ggcc: ResultName[GafisGatherCityCode5202])(rs: WrappedResultSet): GafisGatherCityCode5202 = new GafisGatherCityCode5202(
 ...
 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()
  ...
  def create(
    ...
    seq: Option[Long] = None,
    fingerDataNosqlId: Option[String] = None,
    partitionCitycode: Option[String] = None)(implicit session: DBSession = autoSession): GafisGatherCityCode5202 = {
    withSQL {
      insert.into(GafisGatherCityCode5202).columns(
        column.pkId,
        column.personId,
        column.fgp,
        column.groupId,
  ...
  def save(entity: GafisGatherCityCode5202)(implicit session: DBSession = autoSession): GafisGatherCityCode5202 = {
    withSQL {
      update(GafisGatherCityCode5202).set(
  ...
  def destroy(entity: GafisGatherCityCode5202)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(GafisGatherCityCode5202).where.eq(column.pkId, entity.pkId).and.eq(column.personId, entity.personId).and.eq(column.fgp, entity.fgp).and.eq(column.groupId, entity.groupId).and.eq(column.lobtype, entity.lobtype).and.eq(column.inputpsn, entity.inputpsn).and.eq(column.inputtime, entity.inputtime).and.eq(column.modifiedpsn, entity.modifiedpsn).and.eq(column.modifiedtime, entity.modifiedtime).and.eq(column.fgpCase, entity.fgpCase).and.eq(column.auditStatus, entity.auditStatus).and.eq(column.description, entity.description).and.eq(column.mainPattern, entity.mainPattern).and.eq(column.vicePattern, entity.vicePattern).and.eq(column.gatherData, entity.gatherData).and.eq(column.seq, entity.seq).and.eq(column.fingerDataNosqlId, entity.fingerDataNosqlId).and.eq(column.partitionCitycode, entity.partitionCitycode) }.update.apply()
  }
