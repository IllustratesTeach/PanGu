...
object CodeWhcd extends SQLSyntaxSupport[CodeWhcd] {
 ...
 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()
  ...
  def find(code: String, name: Option[String], deleteFlag: Option[String], ord: Option[Long], remark: Option[String])(implicit session: DBSession = autoSession): Option[CodeWhcd] = {
    withSQL {
      select.from(CodeWhcd as cw).where.eq(cw.code, code).and.eq(cw.name, name).and.eq(cw.deleteFlag, deleteFlag).and.eq(cw.ord, ord).and.eq(cw.remark, remark)
  ...
  def destroy(entity: CodeWhcd)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(CodeWhcd).where.eq(column.code, entity.code).and.eq(column.name, entity.name).and.eq(column.deleteFlag, entity.deleteFlag).and.eq(column.ord, entity.ord).and.eq(column.remark, entity.remark) }.update.apply()
  }
