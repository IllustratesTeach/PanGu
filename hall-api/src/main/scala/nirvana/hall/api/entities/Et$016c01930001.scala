...
case class Et$016c01930001(
  ...
  def save()(implicit session: DBSession = Et$016c01930001.autoSession): Et$016c01930001 = Et$016c01930001.save(this)(session)

  def destroy()(implicit session: DBSession = Et$016c01930001.autoSession): Unit = Et$016c01930001.destroy(this)(session)

}


object Et$016c01930001 extends SQLSyntaxSupport[Et$016c01930001] {

  override val tableName = "ET$016C01930001"
  ...
  def apply(e: SyntaxProvider[Et$016c01930001])(rs: WrappedResultSet): Et$016c01930001 = apply(e.resultName)(rs)
  def apply(e: ResultName[Et$016c01930001])(rs: WrappedResultSet): Et$016c01930001 = new Et$016c01930001(
    pkId = rs.get(e.pkId),
  ...
  val e = Et$016c01930001.syntax("e")

 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()

  def find(pkId: Option[String], personId: Option[String], fgp: Option[Short], groupId: Option[Short], lobtype: Option[Short], inputpsn: Option[String], inputtime: Option[DateTime], modifiedpsn: Option[String], modifiedtime: Option[DateTime], fgpCase: Option[String], auditStatus: Option[Short], description: Option[String], mainPattern: Option[Short], vicePattern: Option[Short], seq: Option[Long], fingerDataNosqlId: Option[String], gatherData: Option[Blob])(implicit session: DBSession = autoSession): Option[Et$016c01930001] = {
    withSQL {
      select.from(Et$016c01930001 as e).where.eq(e.pkId, pkId).and.eq(e.personId, personId).and.eq(e.fgp, fgp).and.eq(e.groupId, groupId).and.eq(e.lobtype, lobtype).and.eq(e.inputpsn, inputpsn).and.eq(e.inputtime, inputtime).and.eq(e.modifiedpsn, modifiedpsn).and.eq(e.modifiedtime, modifiedtime).and.eq(e.fgpCase, fgpCase).and.eq(e.auditStatus, auditStatus).and.eq(e.description, description).and.eq(e.mainPattern, mainPattern).and.eq(e.vicePattern, vicePattern).and.eq(e.seq, seq).and.eq(e.fingerDataNosqlId, fingerDataNosqlId).and.eq(e.gatherData, gatherData)
    }.map(Et$016c01930001(e.resultName)).single.apply()
  }

  def findAll()(implicit session: DBSession = autoSession): List[Et$016c01930001] = {
    withSQL(select.from(Et$016c01930001 as e)).map(Et$016c01930001(e.resultName)).list.apply()
  }

  def countAll()(implicit session: DBSession = autoSession): Long = {
    withSQL(select(sqls.count).from(Et$016c01930001 as e)).map(rs => rs.long(1)).single.apply().get
  }

  def findBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Option[Et$016c01930001] = {
    withSQL {
      select.from(Et$016c01930001 as e).where.append(where)
    }.map(Et$016c01930001(e.resultName)).single.apply()
  }

  def findAllBy(where: SQLSyntax)(implicit session: DBSession = autoSession): List[Et$016c01930001] = {
    withSQL {
      select.from(Et$016c01930001 as e).where.append(where)
    }.map(Et$016c01930001(e.resultName)).list.apply()
  }

  def countBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Long = {
    withSQL {
      select(sqls.count).from(Et$016c01930001 as e).where.append(where)
  ...
  def create(
    ...
    seq: Option[Long] = None,
    fingerDataNosqlId: Option[String] = None,
    gatherData: Option[Blob] = None)(implicit session: DBSession = autoSession): Et$016c01930001 = {
    withSQL {
      insert.into(Et$016c01930001).columns(
        column.pkId,
        column.personId,
        column.fgp,
        column.groupId,
  ...
  def save(entity: Et$016c01930001)(implicit session: DBSession = autoSession): Et$016c01930001 = {
    withSQL {
      update(Et$016c01930001).set(
