...
case class GafisGatherCityCode520(
  pkId: String,
  ...
  def save()(implicit session: DBSession = GafisGatherCityCode520.autoSession): GafisGatherCityCode520 = GafisGatherCityCode520.save(this)(session)

  def destroy()(implicit session: DBSession = GafisGatherCityCode520.autoSession): Unit = GafisGatherCityCode520.destroy(this)(session)

}


object GafisGatherCityCode520 extends SQLSyntaxSupport[GafisGatherCityCode520] {

  override val tableName = "GAFIS_GATHER__CITY_CODE_520"
  ...
  def apply(ggcc: SyntaxProvider[GafisGatherCityCode520])(rs: WrappedResultSet): GafisGatherCityCode520 = apply(ggcc.resultName)(rs)
  def apply(ggcc: ResultName[GafisGatherCityCode520])(rs: WrappedResultSet): GafisGatherCityCode520 = new GafisGatherCityCode520(
  ...
  val ggcc = GafisGatherCityCode520.syntax("ggcc")

 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()

  def find(pkId: String, personId: Option[String], fgp: Short, groupId: Option[Short], lobtype: Short, inputpsn: Option[String], inputtime: DateTime, modifiedpsn: Option[String], modifiedtime: Option[DateTime], fgpCase: Option[String], auditStatus: Option[Short], description: Option[String], mainPattern: Option[Short], vicePattern: Option[Short], gatherData: Blob, seq: Option[Long], fingerDataNosqlId: Option[String], partitionCitycode: Option[String])(implicit session: DBSession = autoSession): Option[GafisGatherCityCode520] = {
    withSQL {
      select.from(GafisGatherCityCode520 as ggcc).where.eq(ggcc.pkId, pkId).and.eq(ggcc.personId, personId).and.eq(ggcc.fgp, fgp).and.eq(ggcc.groupId, groupId).and.eq(ggcc.lobtype, lobtype).and.eq(ggcc.inputpsn, inputpsn).and.eq(ggcc.inputtime, inputtime).and.eq(ggcc.modifiedpsn, modifiedpsn).and.eq(ggcc.modifiedtime, modifiedtime).and.eq(ggcc.fgpCase, fgpCase).and.eq(ggcc.auditStatus, auditStatus).and.eq(ggcc.description, description).and.eq(ggcc.mainPattern, mainPattern).and.eq(ggcc.vicePattern, vicePattern).and.eq(ggcc.gatherData, gatherData).and.eq(ggcc.seq, seq).and.eq(ggcc.fingerDataNosqlId, fingerDataNosqlId).and.eq(ggcc.partitionCitycode, partitionCitycode)
    }.map(GafisGatherCityCode520(ggcc.resultName)).single.apply()
  }

  def findAll()(implicit session: DBSession = autoSession): List[GafisGatherCityCode520] = {
    withSQL(select.from(GafisGatherCityCode520 as ggcc)).map(GafisGatherCityCode520(ggcc.resultName)).list.apply()
  }

  def countAll()(implicit session: DBSession = autoSession): Long = {
    withSQL(select(sqls.count).from(GafisGatherCityCode520 as ggcc)).map(rs => rs.long(1)).single.apply().get
  }

  def findBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Option[GafisGatherCityCode520] = {
    withSQL {
      select.from(GafisGatherCityCode520 as ggcc).where.append(where)
    }.map(GafisGatherCityCode520(ggcc.resultName)).single.apply()
  }

  def findAllBy(where: SQLSyntax)(implicit session: DBSession = autoSession): List[GafisGatherCityCode520] = {
    withSQL {
      select.from(GafisGatherCityCode520 as ggcc).where.append(where)
    }.map(GafisGatherCityCode520(ggcc.resultName)).list.apply()
  }

  def countBy(where: SQLSyntax)(implicit session: DBSession = autoSession): Long = {
    withSQL {
      select(sqls.count).from(GafisGatherCityCode520 as ggcc).where.append(where)
  ...
  def create(
    ...
    seq: Option[Long] = None,
    fingerDataNosqlId: Option[String] = None,
    partitionCitycode: Option[String] = None)(implicit session: DBSession = autoSession): GafisGatherCityCode520 = {
    withSQL {
      insert.into(GafisGatherCityCode520).columns(
        column.pkId,
        column.personId,
        column.fgp,
        column.groupId,
  ...
  def save(entity: GafisGatherCityCode520)(implicit session: DBSession = autoSession): GafisGatherCityCode520 = {
    withSQL {
      update(GafisGatherCityCode520).set(
  ...
  def destroy(entity: GafisGatherCityCode520)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(GafisGatherCityCode520).where.eq(column.pkId, entity.pkId).and.eq(column.personId, entity.personId).and.eq(column.fgp, entity.fgp).and.eq(column.groupId, entity.groupId).and.eq(column.lobtype, entity.lobtype).and.eq(column.inputpsn, entity.inputpsn).and.eq(column.inputtime, entity.inputtime).and.eq(column.modifiedpsn, entity.modifiedpsn).and.eq(column.modifiedtime, entity.modifiedtime).and.eq(column.fgpCase, entity.fgpCase).and.eq(column.auditStatus, entity.auditStatus).and.eq(column.description, entity.description).and.eq(column.mainPattern, entity.mainPattern).and.eq(column.vicePattern, entity.vicePattern).and.eq(column.gatherData, entity.gatherData).and.eq(column.seq, entity.seq).and.eq(column.fingerDataNosqlId, entity.fingerDataNosqlId).and.eq(column.partitionCitycode, entity.partitionCitycode) }.update.apply()
  }
