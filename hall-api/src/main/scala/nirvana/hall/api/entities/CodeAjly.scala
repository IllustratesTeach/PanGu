...
object CodeAjly extends SQLSyntaxSupport[CodeAjly] {
 ...
 override def autoSession = nirvana.hall.api.services.AutoSpringDataSourceSession()
  ...
  def find(code: String, name: Option[String], deleteFlag: Option[String], ord: Option[Long], remark: Option[String])(implicit session: DBSession = autoSession): Option[CodeAjly] = {
    withSQL {
      select.from(CodeAjly as ca).where.eq(ca.code, code).and.eq(ca.name, name).and.eq(ca.deleteFlag, deleteFlag).and.eq(ca.ord, ord).and.eq(ca.remark, remark)
  ...
  def destroy(entity: CodeAjly)(implicit session: DBSession = autoSession): Unit = {
    withSQL { delete.from(CodeAjly).where.eq(column.code, entity.code).and.eq(column.name, entity.name).and.eq(column.deleteFlag, entity.deleteFlag).and.eq(column.ord, entity.ord).and.eq(column.remark, entity.remark) }.update.apply()
  }
