...
import nirvana.hall.v70.internal.adapter.nj.query.QueryGet7to6Service
  ...
  override def getQueryAndSaveMatchResult(queryque: GafisNormalqueryQueryque): Boolean= {
    ...
    if(gafisQuery7to6.isEmpty){
      ...
      try {
        ...
        val matchResult = queryRemoteService.getQuery(gafisQuery7to6.get.queryId, url, headerMap, UUID.randomUUID().toString)
        //逻辑分库处理
        ...
        if (matchResult != null) {
          ...
          val caseSet = new ArrayBuffer[String]()
          val caseFingerSet = new ArrayBuffer[String]()
          val tenprintSet = new ArrayBuffer[String]()
          while (candIter.hasNext) {
            ...
            if (queryque.querytype == QueryConstants.QUERY_TYPE_TT || queryque.querytype == QueryConstants.QUERY_TYPE_LT) {
              ...
              if (GafisPerson.findOption(cand.getObjectId).isEmpty && !tenprintSet.contains(cand.getObjectId)) {
                ...
                tenprintSet += cand.getObjectId
              }
              syncInfoLogManageService.recordSyncDataIdentifyLog(UUID.randomUUID().toString, taskId, HallApiConstants.REMOTE_TYPE_MATCH_RESULT_TPCARD, headerMap.get("X-V62-HOST").get,"0","1")
            } else if (queryque.flag == QueryConstants.FLAG_PALM || queryque.flag == QueryConstants.FLAG_PALM_TEXT) {
              //获取现场信息
              dbid = logicDBJudgeService.logicRemoteJudge(remoteIP, queryque.querytype)
              val cardId = cand.getObjectId
              if (GafisCasePalm.findOption(cardId).isEmpty && !caseFingerSet.contains(cardId)) {
                ...
                lPPalm.foreach { lpPalm =>
                  ...
                  if (caseId != null && caseId.length > 0) {
                    ...
                    if (GafisCase.findOption(caseId).isEmpty && !caseSet.contains(caseId)) {
                      caseInfoRemoteService.isExist(caseId, url, dbId,headerMap)
                ...
                caseFingerSet += lPPalm.get.getStrCardID
              }
              syncInfoLogManageService.recordSyncDataIdentifyLog(UUID.randomUUID().toString, taskId, HallApiConstants.REMOTE_TYPE_MATCH_RESULT_LPPALM, headerMap.get("X-V62-HOST").get,"0","1")
            } else {
              //获取现场信息
              dbid = logicDBJudgeService.logicRemoteJudge(remoteIP, queryque.querytype)
              val cardId = cand.getObjectId
              if (GafisCaseFinger.findOption(cardId).isEmpty && !caseFingerSet.contains(cardId)) {
                val dbId = HttpHeaderUtils.getDBIDBySyncTagert(queryConfig.config, HttpHeaderUtils.DB_KEY_LPLIB)
                ...
                caseFingerSet += lPCard.get.getStrCardID
              }
              syncInfoLogManageService.recordSyncDataIdentifyLog(UUID.randomUUID().toString, taskId, HallApiConstants.REMOTE_TYPE_MATCH_RESULT_LPCARD, headerMap.get("X-V62-HOST").get,"0","1")
            }
          }
          return true
        }
        ...
        case e: nirvana.hall.support.internal.CallRpcException =>
          ...
          error("MatchTask-RequestData fail,uuid:{};taskId:{};错误堆栈信息:{};错误信息:{}",UUID.randomUUID().toString,taskId,eInfo,e.getMessage)
          syncInfoLogManageService.recordSyncDataLog(UUID.randomUUID().toString, taskId, null, eInfo, 2, SyncErrorConstants.SEND_REMOTE_RESPONSE_UNKNOWN + HallApiConstants.REMOTE_TYPE_MATCH_RESULT)
        case e: Exception =>
          val eInfo = ExceptionUtil.getStackTraceInfo(e)
          error("MatchTask-RequestData fail,uuid:{};taskId:{};错误堆栈信息:{};错误信息:{}",UUID.randomUUID().toString,taskId,eInfo,e.getMessage)
          syncInfoLogManageService.recordSyncDataLog(UUID.randomUUID().toString, taskId, null, eInfo, 2, SyncErrorConstants.GET_REMOTE_RESULT_FAIL + HallApiConstants.REMOTE_TYPE_MATCH_RESULT)
      }
      return false
    }
