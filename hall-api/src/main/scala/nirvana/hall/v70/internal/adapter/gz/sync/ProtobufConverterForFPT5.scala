package nirvana.hall.v70.internal.adapter.gz.sync
...
/**
  * Proto与po实体对象相互转换
  */
object ProtobufConverterForFPT5 extends LoggerSupport{
  ...
  def convertGafisCase2Case(caseInfo: GafisCase, fingerIds: Seq[String], palmIds: Seq[String]): Case = {
    ...
    val admData = caseBuilder.getAdmDataBuilder
    magicSet(caseInfo.inputpsn, admData.setCreator)
    magicSet(caseInfo.modifiedpsn, admData.setUpdator)
    magicSet(caseInfo.createUnitCode, admData.setCreateUnitCode)
    magicSet(caseInfo.inputtime, admData.setCreateDatetime)
    magicSet(caseInfo.modifiedtime, admData.setUpdateDatetime)
  ...
  def convertGafisCaseFinger2LPCard(caseFinger: GafisCaseFinger, caseFingerMnt: GafisCaseFingerMnt): LPCard = {
    ...
    val admData = lpCard.getAdmDataBuilder
    magicSet(caseFinger.inputpsn, admData.setCreator)
    magicSet(caseFinger.modifiedpsn, admData.setUpdator)
    magicSet(caseFinger.creatorUnitCode, admData.setCreateUnitCode)
    magicSet(caseFinger.updatorUnitCode, admData.setUpdateUnitCode)
    magicSet(caseFinger.inputtime, admData.setCreateDatetime)
    magicSet(caseFinger.modifiedtime, admData.setUpdateDatetime)
...
//    magicSet(person.personType, textBuilder.setStrPersonType)
    ...
    magicSet(person.assistExplain, textBuilder.setStrXieChaRequestComment)
    ...
    magicSet(person.inputpsn , admData.setCreator)
    magicSet(person.modifiedpsn , admData.setUpdator)
    magicSet(person.gatherOrgCode , admData.setCreateUnitCode)
    magicSet(person.inputtime , admData.setCreateDatetime)
    magicSet(person.modifiedtime , admData.setUpdateDatetime)
...
//      }
    ...
    //指纹数据
    val palmmntMap = palmList.filter(_.groupId == 0).map(f => (f.fgp, f.gatherData)).toMap[Short, Array[Byte]]
    //纹线特征数据
    val palmbinMap = palmList.filter(_.groupId == 4).map(f => (f.fgp, f.gatherData)).toMap[Short, Array[Byte]]
    //掌纹数据
    palmList.filter(_.groupId == 1).foreach {palm =>
      val blobBuilder = tpCard.addBlobBuilder()
      val palmmnt = palmmntMap.get(palm.fgp)
      val palmbin = palmbinMap.get(palm.fgp)
      palmmnt.foreach { blob =>
        blobBuilder.setStMntBytes(ByteString.copyFrom(blob))
      }
      palmbin.foreach{
        blob => blobBuilder.setStBinBytes(ByteString.copyFrom(blob))
      ...
      blobBuilder.setStImageBytes(ByteString.copyFrom(palm.gatherData))
      blobBuilder.setType(ImageType.IMAGETYPE_PALM)
      blobBuilder.setPalmfgp(PalmFgp.valueOf(palm.fgp-10))
    }
    tpCard.build()
  }
  ...
  def convertTPCard2GafisPerson(tpCard: TPCard, person: GafisPerson = new GafisPerson()): GafisPerson={
    ...
    //TODO:证件类别代码 是否需要存库--setStrCertifType
    //TODO:采集人身份证号、采集人联系电话
  ...
  def convertTPCard2GafisGatherFinger(tpCard: TPCard): Seq[GafisGatherFinger] = {
    ...
    while (iter.hasNext){
      ...
      }else if(blob.getType == ImageType.IMAGETYPE_KNUCKLEPRINTS){//指节纹
        val imageData = blob.getStImageBytes
        if(imageData.size() > 0){
          val finger = new GafisGatherFinger()
          finger.personId = personId
          finger.gatherData = imageData.toByteArray
          finger.fgp = blob.getFgp.getNumber.toShort
          finger.fgpCase = Gafis70Constants.FGP_CASE_KNUCKLE_PRINTS
          finger.groupId = Gafis70Constants.GROUP_ID_CPR
          finger.lobtype = Gafis70Constants.LOBTYPE_DATA
          fingerList += finger
        }
  ...
  def convertTPCard2GafisGatherPalm(tpCard: TPCard): Seq[GafisGatherPalm] ={
    ...
    while (blobIter.hasNext){
      ...
      }else if(blob.getType == ImageType.IMAGETYPE_FOURPRINT){
        if (blob.getStImageBytes.size() > 0) {
          val palm = new GafisGatherPalm()
          palm.gatherData = blob.getStImageBytes.toByteArray
          palm.fgp = blob.getPalmfgp match {
            case PalmFgp.PALM_RIGHT => Gafis70Constants.PALM_FOUR_PRINT_RIGHT
            case PalmFgp.PALM_LEFT => Gafis70Constants.PALM_FOUR_PRINT_LEFT
            case PalmFgp.PALM_UNKNOWN => 0
          }
          palm.personId = personId
          palm.groupId = Gafis70Constants.GROUP_ID_CPR
      ...
      }else if(blob.getType == ImageType.IMAGETYPE_FULLPALM){
        if (blob.getStImageBytes.size() > 0) {
          val palm = new GafisGatherPalm()
          palm.gatherData = blob.getStImageBytes.toByteArray
          palm.fgp = blob.getPalmfgp match {
            case PalmFgp.PALM_RIGHT => Gafis70Constants.PALM_FULL_RIGHT
            case PalmFgp.PALM_LEFT => Gafis70Constants.PALM_FULL_LEFT
            case PalmFgp.PALM_UNKNOWN => 0
          }
          palm.personId = personId
          palm.groupId = Gafis70Constants.GROUP_ID_CPR
  ...
  def convertMatchTask2GafisNormalqueryQueryque(matchTask: MatchTask): GafisNormalqueryQueryque = {
    ...
    gafisQuery.computerip = matchTask.getComputerIp
