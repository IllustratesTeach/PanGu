...
/**
  ...
  */
object GafisGatherFingerBak extends ActiveRecordInstance[GafisGatherFingerBak]

@Entity
@Table(name = "GAFIS_GATHER_FINGER_BAK"
)
class GafisGatherFingerBak extends ActiveRecord {
  ...
  def this(gafisGatherFinger:GafisGatherFinger) {
    this()
    this.pkId = gafisGatherFinger.pkId
    this.personId = gafisGatherFinger.personId
    this.fgp = gafisGatherFinger.fgp
    this.groupId = gafisGatherFinger.groupId
    this.lobtype = gafisGatherFinger.lobtype
    this.inputpsn = gafisGatherFinger.inputpsn
    this.inputtime = gafisGatherFinger.inputtime
    this.modifiedpsn = gafisGatherFinger.modifiedpsn
    this.modifiedtime = gafisGatherFinger.modifiedtime
    this.fgpCase = gafisGatherFinger.fgpCase
    this.auditStatus = gafisGatherFinger.auditStatus
    this.description = gafisGatherFinger.description
    this.mainPattern = gafisGatherFinger.mainPattern
    this.vicePattern = gafisGatherFinger.vicePattern
    this.gatherData = gafisGatherFinger.gatherData
    this.seq = gafisGatherFinger.seq
    this.fingerDataNosqlId = gafisGatherFinger.fingerDataNosqlId
  }
}
