...
import nirvana.hall.protocol.v62.lp.CaseAddProto.{CaseAddResponse, CaseAddRequest}
...
/**
  ...
  */
class CaseAddFilter(facade: V62Facade, config: HallV62Config) extends ProtobufRequestFilter{
   override def handle(protobufRequest: BaseRequest, responseBuilder: BaseResponse.Builder, handler: ProtobufRequestHandler): Boolean = {
     if(protobufRequest.hasExtension(CaseAddRequest.cmd)){
