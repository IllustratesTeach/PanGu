   ...
   const int height)       /* image height                 */
{
   ...
   for(cnt = 0; cnt < NUM_SUBBANDS; cnt++) {
      if(quant_vals->var[cnt] < VARIANCE_THRESH)
         ...
         if(cnt < STRT_SIZE_REGION_2 /*4*/)
            quant_vals->qbss[cnt] = 1.0f;
         else
            quant_vals->qbss[cnt] = 10.0f / (A[cnt] *
   ...
   for(cnt = 0; cnt < NUM_SUBBANDS; cnt++){
      if(quant_vals->var[cnt] >= VARIANCE_THRESH){
         ...
         sigma[cnt] = (float)sqrt(quant_vals->var[cnt]);
      }
   }
   K = K1;
   Klen = K0len;
   ...
   while(1){
      ...
      for(i = 0; i < Klen; i++){
         /* Remeber 'm' is the reciprocal of spec. */
         P *= (float)pow((sigma[K[i]] / quant_vals->qbss[K[i]]), m[K[i]]);
      }

      /* Compute new 'q' */
      q = (float)((pow(2,((quant_vals->r/S)-1.0))/2.5) / pow(P, (1.0/S)));
   ...
   for(cnt = 0; cnt < NUM_SUBBANDS; cnt++) {
      if(nK[cnt])
         ...
         quant_vals->qbss[cnt] = 0.0;
      quant_vals->qzbs[cnt] = 1.2f * quant_vals->qbss[cnt];
   }
...
/************************************************************/
void get_lets(
   float *fnew,     /* image pointers for creating subband splits */
   float *old,
   ...
   const int inv)        /* spectral inversion? */
{
   ...
   for(rw_cl = 0; rw_cl < len1; rw_cl++) {
      ...
      if(inv) {
         hipass = fnew + rw_cl * pitch;
         lopass = hipass + hlen * stride;
      }
      else {
         lopass = fnew + rw_cl * pitch;
...
/*************************************************************/
void init_wsq_decoder_resources(DTT_TABLE *pdtt_table)
{
   ...
   pdtt_table->lofilt = (float *)NULL;
   pdtt_table->hifilt = (float *)NULL;
}
...
/*************************************************************/
void free_wsq_decoder_resources(DTT_TABLE *pdtt_table)
{
   if(pdtt_table->lofilt != (float *)NULL){
      free(pdtt_table->lofilt);
      pdtt_table->lofilt = (float *)NULL;
   }

   if(pdtt_table->hifilt != (float *)NULL){
      free(pdtt_table->hifilt);
      pdtt_table->hifilt = (float *)NULL;
   }
}
