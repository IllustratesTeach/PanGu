   ...
   for(cnt = 0; cnt <= a_size; cnt++) {
      ...
      if(dtt_table->hisz % 2) {
         ...
         dtt_table->hifilt[cnt + a_size + 1] = (float)((float)int_sign(cnt)
                                             * a_lofilt[cnt]);
         dtt_table->hifilt[a_size - cnt] = -1.0f *
   ...
   FILE *outfp)      /* compressed file */
{
   ...
   for(coef = (losz>>1); (int)coef < losz; coef++) {
      dbl_tmp = lofilt[coef];
      if(dbl_tmp >= 0.0) {
         sign = 0;
      }
   ...
   for(coef = (hisz>>1); (int)coef < hisz; coef++) {
      dbl_tmp = hifilt[coef];
      if(dbl_tmp >= 0.0) {
         sign = 0;
      }
                    ...
                    const int ilen, unsigned char *comment)
{
   ...
   nalloc = ilen + (sizeof(unsigned short) << 1) + (int)strlen((char *)comment);
   ...
   if (marker == COM_WSQ){
      ...
      do{
         ...
         /* Copy COM segment. */
         if((ret = putc_comment(COM_WSQ, ocomment, (int)strlen((char *)ocomment),
                                ndata, nalloc, &nlen))){
            free(ndata);
            free(ocomment);
            return(ret);
   ...
   if((ret = putc_comment(COM_WSQ, comment, (int)strlen((char *)comment),
                         ndata, nalloc, &nlen))){
      free(ndata);
      return(ret);
   }
   ...
   if((ret = putc_bytes(cbufptr, (int)(ebufptr - cbufptr), ndata, nalloc, &nlen))){
      free(ndata);
      return(ret);
   }
                     ...
                     unsigned char *odata, const int oalloc, int *olen)
{
   ...
   if((ret = putc_comment(COM_WSQ, (unsigned char *)comstr, (int)strlen(comstr),
                        odata, oalloc, olen))){
      freefet(nistcom);
      free(comstr);
      return(ret);
   ...
   if(gencomflag){
      ...
      if((ret = putc_comment(COM_WSQ, (unsigned char *)comment_text,
                             (int)strlen(comment_text), odata, oalloc, olen)))
         return(ret);
   }
