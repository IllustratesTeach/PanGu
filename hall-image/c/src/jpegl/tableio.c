   ...
   FILE *infp)            /* input file */
{
   ...
   if((ret = (int)fread(comment, sizeof(unsigned char), cs, infp)) != cs){
      fprintf(stderr,
   ...
   FILE *outfp)            /* output file */
{
   ...
   if((ret = (int)fwrite(comment, cs, sizeof(unsigned char), outfp)) != cs){
     fprintf(stderr,
                      ...
                      const int ilen, unsigned char *comment)
{
   ...
   if (marker == COM){
      ...
      do{
         ...
         /* Copy COM segment. */
         if((ret = putc_comment(COM, ocomment, (int)strlen((char *)ocomment),
                               ndata, nalloc, &nlen))){
            free(ndata);
            free(ocomment);
            return(ret);
   ...
   if((ret = putc_comment(COM, comment, (int)strlen((char *)comment),
                         ndata, nalloc, &nlen))){
      free(ndata);
      return(ret);
   }
