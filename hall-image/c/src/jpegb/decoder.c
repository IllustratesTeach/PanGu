...
int jpegb_decode_info(int *ow, int *oh, int *od, int *oppi,
					 unsigned char *in_buffer, const int in_buffer_size)
{
	/* This struct contains the JPEG decompression parameters and pointers to
	* working space (which is allocated as needed by the JPEG library).
	...
	*/
	...
	/* Set return pointers. */
	//*oout_buffer = out_buffer;
	*ow = cinfo.image_width;	//output_width;
	*oh = cinfo.image_height;	//output_height;
	*od = cinfo.num_components<<3;	// output_components<<3;
	if((ret = get_ppi_jpegb(oppi, &cinfo))){
		//free(out_buffer);
		//return(ret);
		*oppi = 0;
