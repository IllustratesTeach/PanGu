...
#cat:             from the header into an FET NISTCOM structure.
#cat: get_sd_class - gets the cclass from a special database id field
#cat:
#cat: get_class_from_ncic_class_string - gets the cclass from a special
#cat:             database ncic cclass string
                  ...
                  const int quality)
{
   ...
   if(n_cmpnts > 1){
      ...
      if(strcmp(colorspace, "RGB") == 0)
         cptr = (char *)"1,1:1,1:1,1";
      else if(strcmp(colorspace, "YCbCr") == 0)
         cptr = (char *) "2,2:1,1:1,1";
      else{
...
{
   char *hst, *fname, *class_str, cclass, *sex, *pname;
   int ret, hst_sz;
   NISTCOM *nistcom;
   ...
   if((ret = get_sd_class(ihead->id, 4, &cclass))){
      freefet(nistcom);
      return(ret);
   }
   sprintf(cbuff, "%c", cclass);
...
int sd9_10_14_ihead_to_nistcom(NISTCOM **onistcom, IHEAD *ihead,
                               const int sd_id)
{
   char *hst, *fname, *ncic, cclass, *sex, *pname, *ink_liv;
   char *nptr;
   int ret, hst_sz;
   NISTCOM *nistcom;
   char cbuff[11], id_str[BUFSIZE];
   ...
   ncic = &(id_str[17]);
   nptr = ncic;
   /* put "_" in for " " in the "ncic fpw:??" cases to keep cclass together */
   while(*nptr != '\0') {
      if(*nptr == ' ')
         *nptr = '_';
      nptr++;
...
*/
   ...
   if((ret = get_sd_class(ihead->id, sd_id, &cclass))){
      freefet(nistcom);
      return(ret);
   }
   sprintf(cbuff, "%c", cclass);
...
{
   int ret, n, seqnum;
   char cclass, ncic_str[BUFSIZE];

   if(sd_id == 4){
      if((n = sscanf(id_str, "%*s %c", &cclass)) <= 0){
         fprintf(stderr, "ERROR : get_sd_class : getting cclass");
      ...
      if((ret = get_class_from_ncic_class_string(ncic_str, seqnum, &cclass)))
         return(ret);
   }
