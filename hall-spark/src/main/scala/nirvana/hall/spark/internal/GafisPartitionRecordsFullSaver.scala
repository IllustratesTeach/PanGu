...
import nirvana.hall.spark.services.{SysProperties, PartitionRecordsSaver}
...
class GafisPartitionRecordsFullSaver extends PartitionRecordsSaver {
  ...
  override def savePartitionRecords(parameter: NirvanaSparkConfig)(records: Iterator[(StreamEvent, TemplateFingerConvert, GAFISIMAGESTRUCT, GAFISIMAGESTRUCT)]): Unit = {
    records.foreach{ case(event,image,mnt,bin) =>
        ...
        saveTemplateFingerMntAndBin(image,"0","2",mnt.toByteArray())//object,groupId,lobType,date
        if (bin!=null && parameter.isExtractorBin)
          saveTemplateFingerMntAndBin(image,"4","2",bin.toByteArray())
    }
  }
}
...
object GafisPartitionRecordsFullSaver {
  ...
  def saveLatent(latentCaseConvert: LatentCaseConvert): Unit ={
    ...
    latentCaseConvert.latentFingers.foreach { finger =>
      val hasLatentFinger = updateLatentFinger(finger.fingerId)
      if (hasLatentFinger==0) saveLatentFinger(finger)
      finger.LatentFingerFeatures.foreach{ feature =>
        val hasLatentFingerFeature = updateLatentFingerFeature(feature.fingerId)
  ...
  def updateLatentFinger(fingerId : String) : Int = {
    val updateLatnetFingerSql = "UPDATE gafis_case_finger t SET t.modifiedtime = SYSDATE WHERE t.finger_id = ?"
  ...
  def updateLatentFingerFeature(fingerId : String) : Int = {
    val updateLatnetFingerFeatureSql = "UPDATE gafis_case_finger_mnt t SET t.modifiedtime = SYSDATE WHERE t.finger_id = ? AND t.is_main_mnt = 1"
