...
import nirvana.hall.v70.ln.sys.QueryStatus
import nirvana.hall.webservice.config.HallWebserviceConfig
import nirvana.hall.webservice.internal.haixin.IAConstant
...
//      }
      ...
      fpt.logic02Recs.foreach{
        t => fptService.addLogic02ResHXZC(t)
      }
  ...
  override def getFingerStatus(userid: String, unitcode: String, personid: String): Int = {
    ...
    try{
      ...
      if(!responseStatusAndOraSidMap.nonEmpty){
      ...
      }else{//查询状态:-1未知状态,0等待比对,1正在比对,2等待核查(比对完成),3错误数据';
        if(!responseStatusAndOraSidMap.get("orasid").get.toString.equals(IAConstant.EMPTY_ORASID.toString)){
          ...
          if(oraSid > 0){
            val status = queryStatue.getStatusBySidSQL(oraSid)
